...
export function Wrapper({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <div className="flex min-h-screen dark:bg-gray-900 w-full flex-col bg-muted/40 overflow-hidden">
      <Sidebar />
      <div
        className={`flex flex-col sm:gap-4 sm:pt-4 transition-[padding] duration-300 ease-in-out w-full min-w-0 overflow-hidden ${
          isSidebarOpen ? "sm:pl-20" : "sm:pl-0"
        }`}
      >
        <Topbar />
        <main className="bg-gray-100 dark:bg-gray-900 min-h-screen z-0 w-full min-w-0 overflow-hidden">
          {children}
        </main>
      </div>
    </div>
